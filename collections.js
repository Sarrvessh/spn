(function () {
  ...
  function fieldCard(kind, field, index, fields) {
    ...
    const metaChips = [
      `<span class="field-chip">${esc(typeLabel)}</span>`,
      field.required ? `<span class="field-chip is-required">Required</span>` : "",
      field.sensitive ? `<span class="field-chip is-sensitive">Sensitive</span>` : "",
    ].filter(Boolean).join("");
    const chevronUp = `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 14.5 12 8.5l6 6" fill="none" stroke="currentColor" stroke-width="2.25" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
    const chevronDown = `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 9.5 12 15.5l6-6" fill="none" stroke="currentColor" stroke-width="2.25" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
    return `<article class="field-card" draggable="true" data-field-id="${field.id}"><div class="field-card-header"><div class="field-card-title"><span class="field-index">${index + 1}</span><div class="field-card-copy"><strong>${esc(field.label || typeLabel)}</strong><div class="field-meta">${metaChips}</div></div></div><div class="mini-actions"><div class="field-move-actions"><button class="field-move-button" type="button" title="Move up" aria-label="Move field up" data-field-action="up" data-field-id="${field.id}">${chevronUp}</button><button class="field-move-button" type="button" title="Move down" aria-label="Move field down" data-field-action="down" data-field-id="${field.id}">${chevronDown}</button></div><button class="mini-button" type="button" data-field-action="duplicate" data-field-id="${field.id}">Duplicate</button><button class="mini-button danger-text" type="button" data-field-action="delete" data-field-id="${field.id}">Delete</button></div></div><div class="field-card-body"><div class="field-core"><label><span>Field type</span><select data-field-prop="type" data-field-id="${field.id}">${typeOptions}</select></label><label><span>Field label</span><input data-field-prop="label" data-field-id="${field.id}" value="${esc(field.label)}" placeholder="${esc(typeLabel)}" /></label>${supportsPlaceholder ? `<label><span>Placeholder <small>optional</small></span><input data-field-prop="placeholder" data-field-id="${field.id}" value="${esc(field.placeholder)}" /></label>` : ""}</div><label><span>Helper text <small>optional</small></span><textarea rows="2" data-field-prop="description" data-field-id="${field.id}">${esc(field.description)}</textarea></label>${supportsOptions ? `<label><span>Options <small>one per line</small></span><textarea rows="3" data-field-prop="options" data-field-id="${field.id}">${esc((field.options || []).join("\n"))}</textarea></label>` : ""}${settings ? `<details class="field-card-settings"><summary>More settings</summary><div class="field-settings-body">${settings}</div></details>` : ""}</div></article>`;
  }
